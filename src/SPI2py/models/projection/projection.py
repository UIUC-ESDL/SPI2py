...
def project_component(grid_centers, grid_size,
    ...
    # Apply the kernel to all grid elements. This keeps projection shapes fixed
    # as objects translate and rotate, which is friendlier to JAX/OpenMDAO.
    kernel_points, kernel_radii = apply_kernel(grid_centers, grid_size, kernel_points, kernel_radii)
    ...
    kernel_points_bc = kernel_points.reshape(grid_nx, grid_ny, grid_nz, kernel_count, 1, 3)
    ...
    volume_fractions = (element_overlaps / element_volumes).reshape(grid_nx, grid_ny, grid_nz, kernel_count)

    # Sum fractions to compute pseudo-densities
    all_densities = jnp.sum(volume_fractions, axis=3)
...
def project_capsules(grid_centers, grid_size,
    ...
    # Apply the kernel to all grid elements. This removes object-dependent
    # slicing and gives JAX a stable projection shape.
    kernel_points, kernel_radii = apply_kernel(grid_centers, grid_size, kernel_points, kernel_radii)
